...
/**
 * @brief Checks if last 6 tokens in an array are the same or alternating between two distinct values.
 * 
...
/**
 * @brief Takes a logits tensor and adds the maximum token IDs for each sequence into the newTokens array.
 * 
 * @param geco GECO object for context
 * @param newTokens Array of batchSize-many values to hold the token ID of the most probable next token for each sequence
 ...
 * Otherwise, reshape and bind new input and output tensors based on the previous outputs and run again
 * 
 ...
 * @param nextToks Array holding the next generated tokens from a run. They are used as input to the model.
 * @param batchSize Number of texts being processed at once.
 * @param completed_sequences Array marking if a sequence has completed so we can stop searching for its next tokens.
 */
 ...
 * @param batchSize Number of texts being processed at once
 * 
 * @return OrtValue** The outputs from the decoder model, which will be used as inputs to the decoder_with_past model.
 */
